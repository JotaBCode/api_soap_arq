...
def register_routes(app):
    ...
    def soap_tracking_status():
        ...
        # Depuración: Verificar si los eventos están siendo recuperados correctamente
        if len(events) > 0:
            print(f"Found {len(events)} events for package {package.tracking_number}")
        else:
            print("No events found for this package")
        ...
        if len(events) > 0:
            for event in events:
                ...
                event_date.text = str(event['event_date'])
        ...
        # Asegurarnos de devolver siempre una respuesta válida
